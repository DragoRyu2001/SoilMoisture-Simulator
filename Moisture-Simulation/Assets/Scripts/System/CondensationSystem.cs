...
[BurstCompile]
public partial struct UpdateCloud : IJobEntity
{
    [NativeDisableParallelForRestriction]
    public NativeArray<Color32> texture;
    public Color pixel;
    ...
    public void Execute(ref RainComponent rainComponent)
    {
        int i = (int)(((float)rainComponent.position.x / (float)iLength) * textureWidth * textureWidth);
        int j = (int)((float)rainComponent.position.y / (float)jLength * textureHeight);
        pixel = texture[i + j];

        if (!pixel.Equals(Color.black))
            rainComponent.rain = true;
        else
            rainComponent.rain = false;
    }
}